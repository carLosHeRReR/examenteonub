    ...
    s3.upload(params, (error, data) => {
      if (error) {
        ...
        connection.query(
          ...
          { nombre: nombre, apellido: apellido, salon: salon, imagen: imagenURL },
          (error, results) => {
            if (error) {
              console.log(error);
            } else {
...
app.post('/update/:id', (req, res) => {
  ...
  connection.query(
    'UPDATE estudiantes SET nombre = ?, apellido = ?, salon = ?, imagen = ? WHERE id = ?',
    [nombre, apellido, salon, imagen, id],
    (error, results) => {
      if (error) throw error;
      res.redirect('/');
    }
